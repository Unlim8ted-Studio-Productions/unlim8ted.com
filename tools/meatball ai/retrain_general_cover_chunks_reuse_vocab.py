# retrain_general_cover_chunks_reuse_vocab.py
    ...
    with path.open("r", encoding="utf-8-sig") as f:
        for line_num, line in enumerate(f, start=1):
            ...
            try:
                ...
                print(f"[skip] bad json line {line_num} in {path}: {e}")
                continue
            ...
            if not question or not answer:
                print(f"[skip] missing question/answer line {line_num} in {path.name}")
                continue
...
def vectorize_input(row, input_vocab):
    ...
    unk_idx = input_vocab.get("<UNK>", 1)

    for feat, count in counts.items():
        idx = input_vocab.get(feat, unk_idx)
...
def build_cover_chunks(rows, max_chunk_words=MAX_CHUNK_WORDS):
    tokenized_answers, occurrences, readable = build_candidate_occurrences(
        rows,
        max_chunk_words,
    )
    ...
    while True:
        ...
        for key, occ_list in occurrences.items():
            ...
            if (
                gain > best_gain
                or (gain == best_gain and length > best_len)
                or (gain == best_gain and length == best_len and count > best_count)
            ):
                best_key = key
                best_gain = gain
                best_len = length
...
def normalize_loaded_output_chunks(output_chunks):
    fixed = []
    ...
    for i, chunk in enumerate(output_chunks):
        ...
        c["key"] = str(c["key"])
        c["text"] = str(c["text"])
        c["special"] = bool(c.get("special", False))
        c["count"] = int(c.get("count", 0))
        c["length"] = int(c.get("length", max(1, len(c["key"].split(" ")))))
        fixed.append(c)

    fixed.sort(key=lambda c: c["id"])
...
def load_previous_vocab_and_chunks(reuse_dir: Path):
    input_vocab_path = reuse_dir / "input_vocab.json"
    output_chunks_path = reuse_dir / "output_chunks.json"

    if not input_vocab_path.exists():
        raise FileNotFoundError(f"Missing previous input vocab: {input_vocab_path}")

    if not output_chunks_path.exists():
        raise FileNotFoundError(f"Missing previous output chunks: {output_chunks_path}")

    input_vocab = load_json(input_vocab_path)
    input_vocab = {str(k): int(v) for k, v in input_vocab.items()}

    output_chunks = load_json(output_chunks_path)
    output_chunks = normalize_loaded_output_chunks(output_chunks)

    return input_vocab, output_chunks


def maybe_load_previous_weights(model, reuse_dir: Path):
    candidates = [
        reuse_dir / "model.pt",
        reuse_dir / "general_cover_chunks.pt",
        reuse_dir / "checkpoint.pt",
    ]
    ...
    if model_path is None:
        print("[reuse] no previous .pt weights found; training from random init")
        return False

    print(f"[reuse] loading previous weights: {model_path}")
    ...
    try:
        ...
        print("[reuse] previous weights loaded strict=True")
        return True
    except Exception as e:
        print(f"[reuse] strict load failed: {e}")
        print("[reuse] trying strict=False")

        missing, unexpected = model.load_state_dict(state, strict=False)
        print(f"[reuse] missing keys: {len(missing)}")
        print(f"[reuse] unexpected keys: {len(unexpected)}")
        return True


def print_encoding_stats(rows, output_chunks, key_to_id, max_chunk_words_used):
    encoded_lengths = []
    unk_count = 0
    ...
    multi_chunks = [c for c in real_chunks if int(c["length"]) >= 2]
    single_chunks = [c for c in real_chunks if int(c["length"]) == 1]
    ...
    print(
        f"avg chunks per answer:      {sum(encoded_lengths) / max(len(encoded_lengths), 1):.2f}"
    )
...
def main():
    ...
    parser.add_argument(
        "--out_dir", default="assets/models/general_cover_chunks_test_retrained"
    )

    parser.add_argument(
        "--reuse_dir",
        default="assets/models/general_cover_chunks_test",
        help="Directory containing previous input_vocab.json and output_chunks.json.",
    )
    ...
    parser.add_argument(
        "--rebuild_vocab",
        action="store_true",
        help="Ignore reuse_dir vocab/chunks and rebuild from current data.",
    )

    parser.add_argument(
        "--load_previous_weights",
        action="store_true",
        help="Also initialize from previous model.pt if present in reuse_dir.",
    )

    parser.add_argument(
        "--export_pt",
        action="store_true",
        help="Export model.pt. Enabled automatically in this script.",
    )
    ...
    rows = load_combined_rows(
        Path(args.specialized_dir),
        Path(args.smart_qa_path),
    )
    ...
    if args.rebuild_vocab:
        print("[vocab] rebuilding input vocab and output chunks from current rows")
        output_chunks = build_cover_chunks(rows, max_chunk_words=args.max_chunk_words)
        input_vocab = build_input_vocab(rows)
    else:
        print(f"[vocab] reusing vocab/chunks from: {reuse_dir}")
        input_vocab, output_chunks = load_previous_vocab_and_chunks(reuse_dir)

    key_to_id, max_chunk_words_used = build_chunk_lookup(output_chunks)

    print(f"input vocab size:           {len(input_vocab)}")
    print_encoding_stats(rows, output_chunks, key_to_id, max_chunk_words_used)
    ...
    if args.load_previous_weights and not args.rebuild_vocab:
        maybe_load_previous_weights(model, reuse_dir)
    ...
    print()
    print("[retraining general cover-chunk model]")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch_i, (x, y) in enumerate(train_loader, start=1):
            ...
            if batch_i % 100 == 0:
                print(
                    f"epoch {epoch:03d} batch {batch_i}/{len(train_loader)} | "
                    f"loss {loss.item():.4f}",
                    flush=True,
                )
        ...
        print(
            f"[general-cover-retrain] epoch {epoch:03d} | "
    ...
    config = {
        "model_type": "general_cover_chunk_retrained_reuse_vocab",
        "topic": "general",
        "input_type": "bag_of_words_ngrams",
        "input_ngrams": list(INPUT_NGRAMS),
        "max_output_chunks": MAX_OUTPUT_CHUNKS,
        "chunk_strategy": "reuse_previous_output_chunks",
        "max_chunk_words": int(max_chunk_words_used),
        ...
        "reused_from": str(reuse_dir) if not args.rebuild_vocab else None,
        "rebuilt_vocab": bool(args.rebuild_vocab),
        "loaded_previous_weights": bool(args.load_previous_weights),
    ...
    torch.save(full_pt, out_dir / "model.pt")
    print(f"saved pt: {out_dir / 'model.pt'}")
    ...
    if not args.no_onnx:
        ...
        print(f"saved onnx: {out_dir / 'model.onnx'}")
